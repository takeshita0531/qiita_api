class QiitaMemoryJob < ApplicationJob
  ...
  def perform
    ...
    # sql = 'SELECT title_memo, url_memo, user_memo, id_memo FROM qiita_memories GROUP BY title_memo, url_memo, user_memo, create_time_memo, id_memo HAVING COUNT(*) > 1;'
    ...
    hash = QiitaMemory.group(:url_memo).having('count(*) >= 2').maximum(:id)
    QiitaMemory.where(url_memo: hash.keys).where.not(id: hash.values).destroy_all
